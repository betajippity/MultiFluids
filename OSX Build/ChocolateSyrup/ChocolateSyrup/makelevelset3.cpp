...
static float point_triangle_distance(const glm::vec3 &x0, const glm::vec3 &x1, const glm::vec3 &x2, const glm::vec3 &x3)
{
    ...
    if(w23>=0 && w31>=0 && w12>=0){ // if we're inside the triangle
        ...
        if(w23>0) // this rules out edge 2-3 for us
            return fmin(point_segment_distance(x0,x1,x2), point_segment_distance(x0,x1,x3));
        else if(w31>0) // this rules out edge 1-3
            return fmin(point_segment_distance(x0,x1,x2), point_segment_distance(x0,x2,x3));
        else // w12 must be >0, ruling out edge 1-2
            return fmin(point_segment_distance(x0,x1,x3), point_segment_distance(x0,x2,x3));
    }
}
                     ...
                     Array3f &phi, const int exact_band)
{
    ...
    for(unsigned int t=0; t<tri.size(); ++t){
        ...
        // do distances nearby
        int i0=clamp(int(fmin(fmin(fip,fiq),fir))-exact_band, 0, ni-1), i1=clamp(int(fmax(fmax(fip,fiq),fir))+exact_band+1, 0, ni-1);
        int j0=clamp(int(fmin(fmin(fjp,fjq),fjr))-exact_band, 0, nj-1), j1=clamp(int(fmax(fmax(fjp,fjq),fjr))+exact_band+1, 0, nj-1);
        int k0=clamp(int(fmin(fmin(fkp,fkq),fkr))-exact_band, 0, nk-1), k1=clamp(int(fmax(fmax(fkp,fkq),fkr))+exact_band+1, 0, nk-1);
        ...
        j0=clamp((int)std::ceil(fmin(fmin(fjp,fjq),fjr)), 0, nj-1);
        j1=clamp((int)std::floor(fmax(fmax(fjp,fjq),fjr)), 0, nj-1);
        k0=clamp((int)std::ceil(fmin(fmin(fkp,fkq),fkr)), 0, nk-1);
        k1=clamp((int)std::floor(fmax(fmax(fkp,fkq),fkr)), 0, nk-1);
